...
namespace HellTap.PoolKit {
	...
	public class SpawnerEditor : Editor {
		...
		// Helpers for tabs
		GUIContent[] tabGUIContent = new GUIContent[]{ new(), new(), new() };
		...
		void OnEnable(){
			...
			// Update Tab Sources
			tabGUIContent = new GUIContent[]{ new(), new(), new() };
		...
		public override void OnInspectorGUI(){
			...
			if( _showPoolKitHeaders ){
				HTGUI.Header(headerIcon, "Despawner", "Этот инструмент позволяет вам настроить, как этот игровой объект будет удален. Когда это произойдет, вы можете при желании создать новые объекты на его месте, настроив цепочку создания.", "PoolKit", 80, 13);
			} else {
		...
		void DoDespawnerSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( despawnerIcon, "Despawner", "Эта вкладка позволяет вам настроить, как этот игровой объект будет удален и возвращен в пул. ", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( despawnerIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Despawner", "Установите, как этот игровой объект будет удален." );
					EditorGUILayout.Space();
					...
					if( o.despawnMode == Despawner.DespawnMode.AfterPhysicsCollisionEvent ||
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Source Of Events", "Установите, какой коллайдер следует отслеживать для запуска событий отмены запуска.." );
						EditorGUILayout.Space();
						EditorGUILayout.Space();
					...
					if( o.despawnMode == Despawner.DespawnMode.AfterPhysicsRaycastEvent ||
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Source Of Events", "Установите начальную позицию Raycast." );
						EditorGUILayout.Space();
						EditorGUILayout.Space();
					...
					if( o.despawnMode == Despawner.DespawnMode.AfterPhysicsRaycastEvent ){
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "3D Raycast Settings", "Настройте направление и дистанцию вашей радиопередачи, а также максимальное количество попаданий, которые вы можете проверить. Также доступны опции для проверки триггеров. Вы можете визуализировать радиопередачу в режиме просмотра сцены, желтая линия показывает направление, а красная - расстояние до нее." );
						EditorGUILayout.Space();
						EditorGUILayout.Space();
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Options", "В дополнение к использованию необязательного обратного отсчета времени отмены, вы можете сбросить скорость присоединенного жесткого тела при создании экземпляра этого сборного модуля." );
						EditorGUILayout.Space();
						EditorGUILayout.Space();
					...
					if( o.despawnMode == Despawner.DespawnMode.AfterPhysicsOverlapEvent ){
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Physics Overlap Shape", "Выберите и настройте форму, которую следует использовать для проверки физических столкновений. Вы можете визуализировать форму физического перекрытия в режиме просмотра сцены, она будет выделена красным контуром." );
						EditorGUILayout.Space();
						EditorGUILayout.Space();
					...
					if( o.despawnMode == Despawner.DespawnMode.AfterPhysicsCollisionEvent ){
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Options", "Выберите, какие события уничтожат этот игровой объект. При желании вы можете сбросить скорость для твердых тел при их появлении, а также использовать обратный отсчет в дополнение к физическим событиям." );
						EditorGUILayout.Space();
						EditorGUILayout.Space();
					...
					if( o.despawnMode == Despawner.DespawnMode.AfterPhysicsOverlapEvent ||
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Filter Layers", "Настройте маску слоя, чтобы ограничить коллизии определенными слоями. Только объекты, соответствующие маске слоя, указанной ниже, могут удалить этот объект.");
						EditorGUILayout.Space();
						EditorGUILayout.Space();
						...
						EditorGUILayout.Space();
						HTGUI.SepLine();
						HTGUI.SectionTitle( "Filter Tags", "Вы можете фильтровать столкновения с игровыми объектами с помощью тегов. Только теги, перечисленные ниже, могут удалить этот объект.");
						EditorGUILayout.Space();
						//EditorGUILayout.Space();
		...
		void DoChainSpawningSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( spawnerIcon, "Chain Spawning", "Эта вкладка позволяет вам настроить, как будут создаваться другие префабы, когда этот игровой объект будет удален.", System.String.Empty, _v2_100_64, 13
					);
				...
				if(_chainableSpawn!=null){
					...
					HTGUI.StartWhiteBox();
						...
						EditorGUILayout.BeginHorizontal();
							...
							GUIStyle foldoutGUIStyle = new( GUI.skin.label );
		...
		void DoEventsSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( eventsIcon, "Events", "На этой вкладке можно дополнительно включить делегатов и события Unity. Каждое событие Unity можно включить независимо для оптимизации производительности. ", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( eventsIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Delegates & Events", "Если вы хотите отключить этот параметр, чтобы разрешить делегатам и событиям участвовать в нем, установите флажок ниже." );
					EditorGUILayout.Space();
			...
			//	EditorGUILayout.Space();
				...
				// Title
				HTGUI.SectionTitle( "Unity Events", "Если вы хотите, чтобы это приложение отправляло какие-либо события Unity, активируйте их ниже:" );
				EditorGUILayout.Space();
