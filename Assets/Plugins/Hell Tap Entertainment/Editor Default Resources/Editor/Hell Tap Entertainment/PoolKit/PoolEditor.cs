	...
	public class PoolEditor : Editor {
		...
		public override void OnInspectorGUI(){
			...
			if( _showPoolKitHeaders ){
				HTGUI.Header(headerIcon, "Pool", "Этот инструмент позволяет вам настроить новый пул Pool Kit.", "PoolKit", 80, 13);
			} else {
		...
		void DoPoolSettings(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( poolIcon, "Pool", "На этой вкладке вы можете указать название вашей системы пула и определить, как она работает. Эти параметры являются общими для всех сборных модулей, из которых состоит пул. Если в настройках PoolKit включена функция `Показывать полезные советы`, вам также будет представлен обзор ваших настроек.", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				// Title
				HTGUI.SectionTitle( "Pool Name", "Присвойте этому пулу уникальное имя, чтобы его можно было найти с помощью Polkit API." );
				EditorGUILayout.Space();
		...
		void DoPoolItems(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( prefabIcon, "Prefabs", "На этой вкладке вы можете настроить, какие префабы будут частью этого пула. Несмотря на то, что все эти префабы будут использовать одну и ту же систему пула, они работают независимо друг от друга со своими собственными настраиваемыми параметрами и функциями.", System.String.Empty, _v2_100_64, 13
					);
				...
				for( int i = 0; i < o.poolItems.Length; i++ ){
					...
					if(_poolItem!=null){
                        ...
                        EditorGUILayout.BeginVertical( GUILayout.MinWidth(16), GUILayout.MaxWidth(16), GUILayout.MinHeight(32), GUILayout.MaxHeight(32) );
								...
								GUIStyle boldEventTitleGUIStyle = new(_boldLabel);
							...
							if( _poolItem.tab == 0 ){
								...
								// Prefab Title
								HTGUI.SectionTitle( "Prefab", "Перетащите сборный модуль с панели проекта, чтобы объединить его в пул." );
								EditorGUILayout.Space();
							...
							if( _poolItem.tab == 1 ){
								...
								// Scaling Title
								HTGUI.SectionTitle( "Instance Scale", "Укажите, как следует масштабировать экземпляры при создании." );
								EditorGUILayout.Space();
							...
							if( _poolItem.tab == 2 ){
								...
								// Prefab Title
								HTGUI.SectionTitle( "Lazy Preloading", o.poolType == Pool.PoolType.FixedArray ? "Отложенная предварительная загрузка невозможна для пулов, использующих фиксированный массив." : "Вы можете увеличить пул, предварительно загружая экземпляры с течением времени." );
								EditorGUILayout.Space();
							...
							if( _poolItem.tab == 3 ){
								...
								// Notifications Title
								HTGUI.SectionTitle( "Notifications", "Включение уведомлений может привести к запуску методов `On Spawn` и `Despawn` для ваших экземпляров. Рекомендуется использовать интерфейс `Pool Kit Listener`, поскольку он использует преимущества пользовательского кэширования и является чрезвычайно быстрым. Если вам не нужно вызывать пользовательские методы, вы также можете использовать методы 'OnEnable' и 'OnDisable' в MonoBehaviour." );
								EditorGUILayout.Space();
		...
		public GUIStyle richTextLabelStyle = new();
		public GUIStyle _statisticsHeaderStyle = new();
		...
		void DoPoolStatistics(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( analyticsIcon, "Statistics", "Эта вкладка позволяет вам лучше сбалансировать производительность вашего пула, анализируя его использование. Подробная информация о каждом из ваших префабов и инстансов отображается при запуске игры в редакторе.", System.String.Empty, _v2_100_64, 13
					);
