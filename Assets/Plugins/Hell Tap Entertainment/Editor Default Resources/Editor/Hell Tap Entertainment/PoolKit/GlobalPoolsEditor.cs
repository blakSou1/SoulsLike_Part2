	...
	public class GlobalPoolsEditor : Editor {
		...
		public override void OnInspectorGUI(){
			...
			if( _showPoolKitHeaders){
				HTGUI.Header(headerIcon, "PoolKit Global Pools", "Автоматическое создание списка глобальных пулов при запуске игры.", "PoolKit", 80, 13
					);
			} else {
		...
		void DoSettings(){
			...
			HTGUI.StartWhiteBox();
				...
				// Title
				HTGUI.SectionTitle( "Global Pools", "Перетаскивайте готовые наборы ваших глобальных пулов, чтобы автоматически создавать их при запуске игры." );
				EditorGUILayout.Space();
