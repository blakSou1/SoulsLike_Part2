	...
	public class PoolKitSetupEditor : Editor {
		...
		void DoSettings(){
			...
			HTGUI.StartWhiteBox();
				...
				// Title
				HTGUI.SectionTitle( "PoolKit Setup", "Установите настройки Polkit по умолчанию." );
				EditorGUILayout.Space();
				...
				// Title
				HTGUI.SectionTitle( "Global Pool Group", "Вы можете использовать его в качестве контейнера для своих глобальных пулов, не уничтожая этот игровой объект при смене сцен. Глобальные группы пулов отлично работают, если их сохранить в виде сборных наборов и поместить в первую сцену вашей игры.Если вы хотите настроить `Глобальную группу пулов`, вы должны убедиться, что все пулы, которые являются дочерними для этого игрового объекта, настроены как `глобальные`, установив флажок `Не уничтожать при загрузке` ниже и во всех дочерних пулах."
				 );
				EditorGUILayout.Space();
