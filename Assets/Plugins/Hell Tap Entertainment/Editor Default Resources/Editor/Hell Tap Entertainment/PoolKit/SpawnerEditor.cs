	...
	//	Texture2D[] subtabIcons = new Texture2D[]{ cubeLabel, poolSmallIcon, loadLabel, gearIcon };
		...
		public override void OnInspectorGUI(){
			...
			if( _showPoolKitHeaders ){
				HTGUI.Header(headerIcon, "Spawner", "Этот инструмент позволяет вам настроить новый источник набора для создания пула.", "PoolKit", 80, 13);
			} else {
		...
		void DoSpawnerSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( spawnerIcon, "Spawner", "На этой вкладке вы можете настроить программу запуска, присвоив ей имя и определив ее основные функциональные возможности. Вы можете настроить время запуска программы запуска, продолжительность создания экземпляров и время между каждым появлением. Сборные элементы, точки появления, повороты и масштаб могут независимо обновляться для каждого цикла появления или для каждого экземпляра.", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( spawnerIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Spawner Setup", "Присвойте своему создателю уникальное имя, чтобы сделать его доступным в Polkit API." );
					EditorGUILayout.Space();
		...
		void DoPrefabSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( prefabIcon, "Prefabs", "Эта вкладка позволяет вам настроить, какие префабы будут созданы и какой режим выбора будет использоваться для выбора следующего экземпляра. ", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( prefabIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Prefabs To Spawn", "Настройте, какие префабы будут создаваться и как они будут выбираться." );
					EditorGUILayout.Space();
		...
		void DoInstanceSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( poolIcon, "Instances", "На этой вкладке можно настроить, как настраиваются экземпляры при их создании. Экземпляры можно поворачивать, масштабировать и связывать с другими объектами различными способами.", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( poolIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Instances", "Настройте, как ваши экземпляры будут создаваться, масштабироваться и поворачиваться при создании." );
					EditorGUILayout.Space();
		...
		void DoLocationSetup(){
			...
			if( _showTabHeaders ){
				...
				HTGUI.StartWhiteBox();
					...
					// Title
					EditorGUILayout.Space();
					HTGUI.Header( spawnPointIcon, "Spawn Points", "На этой вкладке вы можете настроить, где будут создаваться экземпляры. Вы можете создавать экземпляры непосредственно в программе создания, используя список объектов Transform или список позиций Vector3. Существует множество опций для управления каждым режимом, а также для выбора различных способов выбора следующей точки появления.", System.String.Empty, _v2_100_64, 13
					);
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( spawnPointIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Spawn Locations", "Настройте, где будут создаваться ваши экземпляры." );
					EditorGUILayout.Space();
				...
				// Title
				HTGUI.SectionTitle( "Randomized Offsets", "К указанным выше позициям точек появления могут быть применены случайные смещения." );
				EditorGUILayout.Space();
				...
				if( o.addSpawnPointRandomizationRange ){
					...
					// Title
					HTGUI.SectionTitle( "Clamp Randomized Offsets", "Рандомизированные смещения могут быть ограничены минимальной и максимальной величиной." );
					EditorGUILayout.Space();
		...
		void DoEventsSetup(){
			...
			HTGUI.StartWhiteBox();
				...
				StartIconColumn( eventsIcon, 64, 64, 4 );
					...
					// Title
					HTGUI.SectionTitle( "Delegates & Events", "Если вы хотите, чтобы этот инструмент создания разрешал делегатам участвовать в мероприятиях, установите флажок ниже." );
					EditorGUILayout.Space();
			...
			//	EditorGUILayout.Space();
				...
				// Title
				HTGUI.SectionTitle( "Unity Events", "Если вы хотите, чтобы этот спаунер отправлял какие-либо события Unity, активируйте их ниже:" );
				EditorGUILayout.Space();
