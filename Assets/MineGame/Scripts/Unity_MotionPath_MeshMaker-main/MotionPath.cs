    ...
    void DrawPathInfo(MotionPath Ge)
    {
        ...
        for (int i = 0; i < PathInfo.Count; i++)
        {
            ...
            else
            {
                ...
                {
                    Handles.Label(PathInfo[i].VertexPos[j], EditorGUIUtility.IconContent("d_Record On"), Style_Vert); //버텍스 위치들
                }
            }
            ...
            {
                Handles.Label(PathInfo[i].TargetObject.transform.position, EditorGUIUtility.IconContent("d_Record Off"), Style_Target); //버텍스 위치들
            }
        }
    }
