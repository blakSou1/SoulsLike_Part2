...
public class LockOnComponent : MonoBehaviour
{
    ...
    private float TimeRealUpdateTarget = .2f;
    ...
    private IEnumerator TickIpdateTarget()
    {
        yield return new WaitForSeconds(TimeRealUpdateTarget);
    ...
    public void LockOn()
    {
        ...
        {
            FindLockableTarget();
            CurrentLockable = SelecktBestTarget(_potentialTargets);
            ...
            if (CurrentLockable != null)
            {
                ...
                StartCoroutine(TickIpdateTarget());
    ...
    #region Sub
    private ILockable SelecktBestTarget(List<ILockable> targetLict)
    {
        ILockable bestTarget = null;
        float maxScore = -Mathf.Infinity;

        foreach (ILockable target in targetLict)
        {
    ...
    public void SwitchTarget(int direction)
    {
        ...
        List<ILockable> targetsList = targetsWithAngles.Select(t => t.Target).ToList();

        ILockable bestTarget = SelecktBestTarget(targetsList);

        if (bestTarget != null)
        {
            CurrentLockable = bestTarget;
            G.playerView.motionWarpingSystem.currentTarget = bestTarget.GetLockOnTarget();

            FollowCinemachine.LookAt = bestTarget.GetLockOnTarget();
        }
    }
