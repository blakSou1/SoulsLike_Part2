...
public class PlayerMovementComponent : IDisposable
{
    ...
    private void Movement()
    {
        ...
        //HANDLE SPEED
        if (G.playerView.LockOnComponent.lockOn && !isSprint)
        {
            targetVelocity = movementSpeed * inputMoveDirection.y * G.playerView.transform.forward;
            targetVelocity += movementSpeed * inputMoveDirection.x * G.playerView.transform.right;
        }
        else
        {
    ...
    {
        G.playerView.AnimHook.Anim.SetBool("isSprint", isSprint);
